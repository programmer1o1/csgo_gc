...
public:
    void FireGameEvent(IGameEvent *event) override
    {
        ...
        {
            // Resend SO cache unconditionally — no userId needed.
            // OnLoadoutChanged fires when CCSPlayerInventory exists (round start).
...
static void *Hk_SteamInternal_FindOrCreateUserInterface(HSteamUser hSteamUser, const char *pszVersion)
{
    ...
    if (strcmp(pszVersion, STEAMGAMECOORDINATOR_INTERFACE_VERSION) == 0)
    {
        ...
        if (!s_cs2GCProxyServer)
        {
            Platform::Print("csgo_gc: intercepted server GC via FindOrCreateUserInterface\n");
            s_cs2GCProxyServer = new SteamGameCoordinatorProxy(0);
        }
        return s_cs2GCProxyServer;
    }
