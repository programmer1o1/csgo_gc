...
NetworkingServer::NetworkingServer([[maybe_unused]] ISteamNetworkingMessages *networkingMessages)
...
void NetworkingServer::ClientConnected(uint64_t steamId, const void *ticket, uint32_t ticketSize)
{
    ...
    SendMessageToUser(Messages(), steamId, messageWrite.Data(), messageWrite.Size());
}
...
void NetworkingServer::SendMessage(uint64_t steamId, const void *data, uint32_t size)
{
    ...
    SendMessageToUser(Messages(), steamId, data, size);
}
